...
process_execute (const char *file_name) 
{
  ...
  if (fn_copy == NULL || file_name_copy == NULL)
    return TID_ERROR;
  ...
  char *first_word;
  char *remain;
  first_word = strtok_r(file_name_copy, " ", &remain);
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  //page directory는 프로세스의 가상 메모리 & 실제 physical memory mapping
  t->pagedir = pagedir_create ();
  if (t->pagedir == NULL) 
    goto done;
  process_activate ();

  /*student code : parsing before open*/
  int argc = 0;
  char *delim;
  ...
  strlcpy(file_name_copy, file_name, strlen(file_name) + 1);
  while((delim = strchr(file_name_copy, ' '))){
    if (*file_name_copy == ' '){
      file_name_copy++;
      continue;
    }
    argv[argc++] = file_name_copy;
    *delim = '\0';
    file_name_copy = delim + 1;
  }
  if (*file_name_copy != '\0') {
    argv[argc] = file_name_copy;
  }
  ...
  char **mystack = (char**)malloc(sizeof(char*)*(argc+1));
  *esp = PHYS_BASE;
  ...
  if ((alignment = count % 4)) { //4의 배수가 아닐 때
    *esp -=  4 - alignment;
  }
  *esp -= 4;
  **((uint32_t**)esp) = 0;
  for(int i = argc; i >=0; i--){
    *esp -= 4;
    **((uint32_t**)esp) = (uint32_t)mystack[i];
  }
  *esp -= 4;
